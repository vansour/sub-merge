...
- `crates/server`：axum 服务。SQLite（sqlx）持久化 sources/users/sessions/combined_subs/combined_sources/settings（settings 存 clash 模板等键值）。路由：`/subscribe/{name}`（组合订阅输出，clash 为订阅组模式——模板+provider 引用聚合链接，v2ray/singbox 按成员拉取解析，无鉴权）、`/admin/*`（源 CRUD/预览/配置/Clash 配置/组合订阅 CRUD，Bearer 鉴权）；`/healthz`；SPA 静态托管 + 前端路由回退。
- `crates/server/web`（submerge-web）：**独立于 workspace**（不加入 members，避免 dx build 与 cargo build --workspace 冲突），只能由 dx 构建。dioxus 0.8.0-alpha.1（精确版本锁定）+ WASM。样式全部在 `index.html` 内联 CSS（CSS 变量双主题，`prefers-color-scheme` 自动切换）。页面数据经 `src/data.rs` 的 DataStore 单元缓存（按 sources/combineds/preview/config/clash_config 五单元共享）：MainShell 先预载再切换 tab（旧页保持+菜单项转圈），已访问页缓存秒开；CRUD/刷新后 `refresh` 对应单元回写缓存。
